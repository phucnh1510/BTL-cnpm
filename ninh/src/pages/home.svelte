<script>
  ...
  onMount(() => {
    if (!$accessTokenStore.userId) {
      navigate("/login");
    }
    const url = "http://localhost:5292/api/sv/problemset?svId=" + $accessTokenStore.userId;
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
        ...
        {#if paginatedProblems.length > 0}
          {#each paginatedProblems as problem}
            <div class="table-row">
              ...
              <button class="table-column title-column" on:click={HW}>{problem.title}</button>
